  ...
  for (let iter = 0; iter < maxIterations; iter++) {
    ...
    for (let c = 0; c < k; c++) {
      ...
      if (clusterPoints.length > 0) {
        centroids[c] = categories.map((_, catIdx) => clusterPoints.reduce((sum, p) => sum + p[catIdx], 0) / clusterPoints.length);
      }
    }
  }
...
export const AggregateNetworkVisualization = ({
  ...
  const similarityMetrics = useMemo(() => {
    ...
    const categoryAverages = categories.map(cat => {
      const avg = fingerprints.reduce((sum, fp) => sum + (Number(fp[cat.key as keyof UserFingerprint]) || 0), 0) / fingerprints.length;
      return {
        ...cat,
        avg
      };
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < fingerprints.length; i++) {
      for (let j = i + 1; j < fingerprints.length; j++) {
        const similarity = calculateSimilarity(fingerprints[i], fingerprints[j]);
        const sameCluster = userClusterMap.get(fingerprints[i].user_id)?.id === userClusterMap.get(fingerprints[j].user_id)?.id;
    ...
    // Background gradient
    const bgGradient = defs.append("radialGradient").attr("id", "aggregate-bg-gradient");
    ...
    // Background
    svg.append("rect").attr("width", width).attr("height", height).attr("fill", "url(#aggregate-bg-gradient)");
    ...
    linkGroup.selectAll("line").data(links).join("line").attr("stroke", d => d.sameCluster ? d.source.color : "hsl(var(--muted-foreground))").attr("stroke-opacity", d => d.sameCluster ? 0.4 : 0.15).attr("stroke-width", d => d.sameCluster ? 2 : 1).attr("stroke-dasharray", d => d.sameCluster ? "none" : "4 4");

    // Draw nodes
    const nodeGroup = svg.append("g").attr("class", "nodes");
    const nodeElements = nodeGroup.selectAll("g").data(nodes).join("g").attr("cursor", "pointer").call(d3.drag<any, any>().on("start", (event, d) => {
    ...
    nodeElements.append("circle").attr("r", d => d.radius + 12).attr("fill", "none").attr("stroke", d => d.color).attr("stroke-width", 3).attr("stroke-dasharray", "6 3").attr("opacity", 0.5);

    // Glow effect
    nodeElements.append("circle").attr("r", d => d.radius + 8).attr("fill", d => d.color).attr("opacity", 0.15);

    // Main circle
    nodeElements.append("circle").attr("r", d => d.radius).attr("fill", d => d.color).attr("stroke", "hsl(var(--background))").attr("stroke-width", 2);

    // User initial
    nodeElements.append("text").attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("fill", "white").attr("font-weight", "bold").attr("font-size", d => d.radius * 0.6).text(d => d.fingerprint.username?.charAt(0).toUpperCase() || "U");

    // Username label
    nodeElements.append("text").attr("text-anchor", "middle").attr("y", d => d.radius + 20).attr("fill", "hsl(var(--foreground))").attr("font-size", 11).attr("font-weight", 500).text(d => d.fingerprint.username || "User");

    // Source count badge
    nodeElements.append("circle").attr("cx", d => d.radius * 0.7).attr("cy", d => -d.radius * 0.7).attr("r", 12).attr("fill", "hsl(var(--background))").attr("stroke", d => d.color).attr("stroke-width", 2);
    nodeElements.append("text").attr("x", d => d.radius * 0.7).attr("y", d => -d.radius * 0.7).attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("fill", "hsl(var(--foreground))").attr("font-size", 9).attr("font-weight", "bold").text(d => d.fingerprint.total_sources_analyzed);
    ...
    nodeElements.on("mouseenter", function (event, d) {
      ...
      d3.select(this).select("circle:nth-child(3)").transition().duration(200).attr("r", d.radius * 1.2);
    }).on("mouseleave", function (event, d) {
      setHoveredUser(null);
      setHoveredCluster(null);
      d3.select(this).select("circle:nth-child(3)").transition().duration(200).attr("r", d.radius);
    }).on("click", (event, d) => {
      if (onUserClick) onUserClick(d.id);
    });

    // Force simulation with cluster forces
    const simulation = d3.forceSimulation(nodes as any).force("link", d3.forceLink(links).distance((d: any) => d.sameCluster ? 80 : 150)).force("charge", d3.forceManyBody().strength(-250)).force("center", d3.forceCenter(width / 2, height / 2)).force("collision", d3.forceCollide().radius((d: any) => d.radius + 25))
      ...
      clusters.forEach(cluster => {
        ...
        if (clusterNodes.length >= 3) {
          ...
          if (hull) {
            ...
            hullGroup.append("path").attr("d", `M${expandedHull.join("L")}Z`).attr("fill", cluster.color).attr("fill-opacity", 0.08).attr("stroke", cluster.color).attr("stroke-opacity", 0.3).attr("stroke-width", 2).attr("stroke-dasharray", "8 4");
          }
        }
      });
  ...
  return <div className="space-y-6">
      <Card className="relative overflow-hidden bg-gradient-to-br from-card via-card to-card/80 border-border/50">
        ...
        {clusters.length > 0 && <div className="absolute top-4 right-4 z-10 bg-card/90 backdrop-blur-sm rounded-lg p-3 border border-border/50">
            <div className="flex items-center gap-2 mb-2">
              <Layers className="h-4 w-4 text-muted-foreground" />
              <span className="text-xs font-semibold text-foreground">Communities</span>
            </div>
            <div className="space-y-1">
              {clusters.map(cluster => <div key={cluster.id} className="flex items-center gap-2 text-xs" onMouseEnter={() => setHoveredCluster(cluster)} onMouseLeave={() => setHoveredCluster(null)}>
        ...
        {/* Hover tooltip */}
        {hoveredUser && <div className="fixed z-50 bg-popover border border-border rounded-lg shadow-lg p-3 pointer-events-none" style={{
        ...
        maxWidth: 280
            ...
            {hoveredCluster && <div className="flex items-center gap-2 mb-2 px-2 py-1 rounded-full text-xs" style={{
          backgroundColor: `${hoveredCluster.color}20`
        }}>
                <div className="w-2 h-2 rounded-full" style={{
            backgroundColor: hoveredCluster.color
          }} />
                <span style={{
            color: hoveredCluster.color
          }}>{hoveredCluster.label}</span>
              </div>}
            <div className="grid grid-cols-2 gap-1 text-xs">
              {categories.map(cat => <div key={cat.key} className="flex items-center gap-1">
                  <div className="w-2 h-2 rounded-full" style={{
      ...
      {/* Cluster Analysis */}
      {clusters.length > 0 && <Card className="p-6 bg-card/80">
          <div className="flex items-center gap-2 mb-4">
            <Layers className="h-5 w-5 text-primary" />
            <h4 className="font-semibold text-foreground opacity-90">Community Clusters</h4>
          </div>
          
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {clusters.map(cluster => {
          const Icon = cluster.dominantCategory.icon;
          return <div key={cluster.id} className="p-4 rounded-lg border-2 transition-colors" style={{
            borderColor: cluster.color,
            backgroundColor: `${cluster.color}08`
                ...
                backgroundColor: `${cluster.color}20`
                  ...
                  <div className="space-y-1 mb-3">
                    {categories.map((cat, idx) => <div key={cat.key} className="flex items-center gap-2 text-xs">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {cluster.members.slice(0, 6).map(member => <Avatar key={member.user_id} className="h-6 w-6 border-2" style={{
                borderColor: cluster.color
              }}>
                    ...
                    {cluster.members.length > 6 && <div className="h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-medium" style={{
                backgroundColor: `${cluster.color}20`,
                color: cluster.color
              }}>
      ...
      {/* Similarity Metrics */}
      {similarityMetrics && <Card className="p-6 bg-card/80">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              width: `${similarityMetrics.averageSimilarity * 100}%`
            }} />
              </div>
              ...
              <div className="mt-4">
                ...
                <div className="space-y-1">
                  {similarityMetrics.pairs.slice(0, 3).map((pair, i) => <div key={i} className="flex items-center justify-between text-sm">
            ...
            const Icon = cat.icon;
            return <div key={cat.key} className="flex items-center gap-2">
                    <Icon className="h-4 w-4" style={{
                color: cat.color
              }} />
      ...
      <Card className="p-4 bg-card/50">
        <div className="flex flex-wrap items-center justify-center gap-4 text-sm">
          {categories.map(cat => {
          const Icon = cat.icon;
          return <div key={cat.key} className="flex items-center gap-1.5">
