...
// src/lib/mcp/tools/hear.ts
import { defineTool as defineTool5 } from "npm:@lovable.dev/mcp-js@0.28.0";
import { z as z4 } from "npm:zod@^3.25.76";
var hear_default = defineTool5({
  name: "hear",
  title: "Hear a sound (fingerprint, archetype, signature)",
  description: "Score a sound with SONICSIM. Pass `source_id` (an analysed audio source) or `url` (a source already in the library), or `text` describing the sound. Returns the six-category fingerprint, the nearest Ensemble archetype and the sonic signature reference. Disabled until an administrator enables the hear API.",
  inputSchema: {
    source_id: z4.string().describe("UUID of an analysed audio source.").optional(),
    url: z4.string().describe("URL of an audio source already in the library.").optional(),
    text: z4.string().describe("Text or comma-separated tags describing the sound.").optional()
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
  handler: async (input, ctx) => {
    if (!ctx.isAuthenticated()) return notAuthenticated;
    const supabase = supabaseForUser(ctx);
    const { data: flag } = await supabase.from("control_registry").select("value").eq("key", "nextlevel.hear_api_enabled").maybeSingle();
    if (flag?.value !== true) {
      return {
        content: [
          {
    ...
    const text = [
      ...
      signature ? `Archetype ${signature.archetype_slug} (distance ${signature.distance}) \xB7 signature ${signature.subject_hash}` : "No signature rendered yet."
    ].join("\n");
    return {
...
var mcp_default = defineMcp({
  ...
  tools: [
    ...
    get_fingerprint_default,
    list_audio_sources_default,
    search_taxonomy_default,
    hear_default
  ]
});
