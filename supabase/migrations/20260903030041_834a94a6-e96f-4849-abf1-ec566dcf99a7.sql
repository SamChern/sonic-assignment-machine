CREATE OR REPLACE FUNCTION public.intuizi_tag_signature(p_report_type text, p_tags jsonb)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT encode(
    digest(
      coalesce(p_report_type, '') || '|' || coalesce((
        SELECT string_agg(c, ',' ORDER BY c)
        FROM (
          SELECT DISTINCT (t->>'code') AS c
...
BEGIN
  ...
  FOR r IN
    SELECT q.id, q.identifier, q.report_type, q.tags, q.label, q.confidence,
     ...
     WHERE q.activation_id = p_activation_id
  ...
  SELECT v_count,
         (SELECT count(*)::integer FROM (
            SELECT 1 FROM public.intuizi_score_queue
             WHERE activation_id = p_activation_id AND status = 'pending' LIMIT 100000
          ) s);
END;
$$;

REVOKE ALL ON FUNCTION public.materialize_cached_intuizi_scores(text, integer) FROM PUBLIC, anon;
