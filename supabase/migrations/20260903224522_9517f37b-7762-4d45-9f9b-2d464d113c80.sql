-- Keep activation_id always populated on the scoring queue -------------------
CREATE OR REPLACE FUNCTION public.intuizi_queue_set_activation_id()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF NEW.activation_id IS NULL AND NEW.object_key IS NOT NULL THEN
    NEW.activation_id := (regexp_match(NEW.object_key, 'activation[_-]?id([0-9]+)', 'i'))[1];
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_intuizi_queue_activation_id ON public.intuizi_score_queue;
CREATE TRIGGER trg_intuizi_queue_activation_id
BEFORE INSERT OR UPDATE OF object_key ON public.intuizi_score_queue
FOR EACH ROW EXECUTE FUNCTION public.intuizi_queue_set_activation_id();

CREATE OR REPLACE FUNCTION public.backfill_intuizi_activation_ids(p_limit integer DEFAULT 200000)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  WITH c AS (
    SELECT id, (regexp_match(object_key, 'activation[_-]?id([0-9]+)', 'i'))[1] AS act
    FROM public.intuizi_score_queue
    WHERE activation_id IS NULL
      AND object_key ~* 'activation[_-]?id[0-9]+'
    LIMIT GREATEST(1, LEAST(p_limit, 500000))
  )
  UPDATE public.intuizi_score_queue q
  SET activation_id = c.act
  FROM c
  WHERE q.id = c.id;
...
-- Build / refresh one activation's aggregate audience profile ----------------
...
BEGIN
  ...
  -- Owner: whoever ingested the activation, else any admin.
  ...
  -- Reuse the existing profile source when one is already linked.
  ...
  -- Tag weights: share of sampled identifiers carrying each tag code.
  ...
  -- Aggregate the activation's already-scored identifiers (no AI spend).
  ...
  IF v_scored > 0 THEN
    ...
    INSERT INTO public.source_analyses (
      ...
      confidence, category, grounding_level, raw_scores
    ) VALUES (
  ...
  -- Link the activation profile identifier row.
