CREATE OR REPLACE FUNCTION public.build_activation_profile(p_activation text, p_sample integer DEFAULT 40000, p_top_tags integer DEFAULT 40)
 RETURNS TABLE(audio_source_id uuid, tags_written integer, identifiers_seen integer, scored_identifiers integer, analysis_written boolean, category text, confidence numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Tie the profile to the workspace that owns this activation so the
  -- enterprise workspace can see it (workspace views filter on organization_id).
  SELECT organization_id INTO v_org
  FROM public.org_intuizi_activations
  WHERE activation_id = p_activation AND coalesce(is_active, true) = true
  ...
  IF v_source IS NULL THEN
    INSERT INTO public.audio_sources (user_id, organization_id, source_type, name, analysis_status, ctv_metadata)
    VALUES (
      v_owner,
      v_org,
      'intuizi',
  ...
  IF v_scored > 0 THEN
    ...
    INSERT INTO public.source_analyses (
      user_id, audio_source_id, organization_id, source_name,
      ...
      v_owner, v_source, v_org, 'Activation ' || p_activation || ' audience profile',
...
-- Backfill: link existing activation profiles to the workspace that owns them.
UPDATE public.audio_sources s
SET organization_id = a.organization_id
FROM public.org_intuizi_activations a
WHERE s.organization_id IS NULL
  AND s.source_type = 'intuizi'
  AND s.ctv_metadata->>'scope' = 'activation_profile'
  AND s.ctv_metadata->>'activation_id' = a.activation_id
  AND coalesce(a.is_active, true) = true;

UPDATE public.source_analyses an
SET organization_id = s.organization_id
FROM public.audio_sources s
WHERE an.audio_source_id = s.id
