ALTER TABLE public.intuizi_score_queue SET (autovacuum_vacuum_scale_factor = 0.02, autovacuum_analyze_scale_factor = 0.02, autovacuum_vacuum_cost_limit = 1000);

CREATE OR REPLACE FUNCTION public.intuizi_activation_cost_estimate(p_sample integer DEFAULT 1000, p_force boolean DEFAULT false, p_max_age_seconds integer DEFAULT 900)
 RETURNS TABLE(activation_id text, total_rows integer, done_rows integer, pending_rows integer, sampled_rows integer, sampled_cached_rows integer, distinct_signatures integer, billable_signatures integer, computed_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Recompute, bounded. If the queue is too busy the previous snapshot is kept
  -- and the caller still receives the last good numbers instead of an error.
  BEGIN
  ...
  EXCEPTION
    WHEN query_canceled OR deadlock_detected OR lock_not_available THEN
      NULL;
  END;
