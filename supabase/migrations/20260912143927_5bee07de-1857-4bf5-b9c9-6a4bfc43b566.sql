CREATE TABLE IF NOT EXISTS public.grounding_rescore_sweeps (
...
GRANT SELECT ON public.grounding_rescore_sweeps TO authenticated;
GRANT ALL ON public.grounding_rescore_sweeps TO service_role;
ALTER TABLE public.grounding_rescore_sweeps ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins read grounding sweeps" ON public.grounding_rescore_sweeps;
CREATE POLICY "Admins read grounding sweeps"
  ON public.grounding_rescore_sweeps FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE UNIQUE INDEX IF NOT EXISTS idx_grounding_sweep_one_active
  ON public.grounding_rescore_sweeps (singleton)
...
BEGIN
  ...
  -- Single flight: only a sweep that is running and whose lease has expired runs.
...
REVOKE ALL ON FUNCTION public.grounding_rescore_tick(integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.grounding_rescore_tick(integer) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.admin_grounding_rescore_control(_action text, _batch integer DEFAULT 100)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  IF _action = 'start' THEN
    UPDATE public.grounding_rescore_sweeps
    SET status = 'running', lease_until = NULL, note = NULL,
        batch_size = LEAST(GREATEST(COALESCE(_batch, 100), 1), 500), updated_at = now()
    WHERE status IN ('running','paused')
    RETURNING * INTO v_row;

    IF v_row.id IS NULL THEN
      INSERT INTO public.grounding_rescore_sweeps (status, batch_size)
      VALUES ('running', LEAST(GREATEST(COALESCE(_batch, 100), 1), 500))
      RETURNING * INTO v_row;
    END IF;

  ELSIF _action = 'restart' THEN
    UPDATE public.grounding_rescore_sweeps SET status = 'done', updated_at = now()
    WHERE status IN ('running','paused');
    INSERT INTO public.grounding_rescore_sweeps (status, batch_size)
    VALUES ('running', LEAST(GREATEST(COALESCE(_batch, 100), 1), 500))
    RETURNING * INTO v_row;
  ...
  ELSIF _action = 'pause' THEN
    UPDATE public.grounding_rescore_sweeps
    SET status = 'paused', lease_until = NULL, updated_at = now()
...
REVOKE ALL ON FUNCTION public.admin_grounding_rescore_control(text, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_grounding_rescore_control(text, integer) TO authenticated;
...
BEGIN
  ...
  FROM public.audio_sources a
  WHERE a.profile_embedding IS NOT NULL
    AND (v_row.last_source_id IS NULL OR v_row.status NOT IN ('running','paused') OR a.id > v_row.last_source_id);

  SELECT count(*)::int INTO v_upgradable
  FROM public.source_analyses sa
  ...
  RETURN jsonb_build_object(
    ...
    'queue_remaining', v_remaining,
    'text_only_with_vector', v_upgradable,
    'computed_at', now());
END;
$function$;

REVOKE ALL ON FUNCTION public.admin_grounding_rescore_status() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_grounding_rescore_status() TO authenticated;